#!/usr/bin/env python3
"""
Script para coletar TODOS os countries da Sportmonks API com paginação completa
"""
...
def get_all_countries_with_pagination():
    """Buscar TODOS os countries da API Sportmonks com paginação completa"""
    ...
    logger.info("🌐 Iniciando coleta COMPLETA com paginação...")
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if countries:
                    logger.info(f"   📋 Exemplos: {countries[0].get('name')} ({countries[0].get('iso2')}), {countries[1].get('name') if len(countries) > 1 else 'N/A'}")
        ...
        if page > 50:  # Máximo de 50 páginas (1250 countries)
            logger.warning("⚠️ Limite de páginas atingido (50)")
            break
    
    logger.info(f"🎉 Coleta COMPLETA finalizada: {len(all_countries)} countries coletados")
...
def generate_final_report():
    ...
    try:
        ...
        continent_names = {1: "Europa", 2: "América do Norte", 3: "Ásia", 4: "América do Sul", 5: "África", 6: "Oceania", 7: "Outros"}
        ...
        # Top 10 países por ID (mais antigos na API)
        logger.info("\n🏆 Top 10 países (IDs mais baixos):")
        top_countries = supabase.table('countries').select('sportmonks_id,name,iso2,fifa_name').order('sportmonks_id').limit(10).execute()
        for country in top_countries.data:
            logger.info(f"   • ID {country['sportmonks_id']}: {country['name']} ({country['iso2']}) - FIFA: {country['fifa_name']}")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("🚀 COLETANDO TODOS OS COUNTRIES DA SPORTMONKS API COM PAGINAÇÃO COMPLETA")
    logger.info("=" * 80)
    
    # Coletar todos os countries com paginação completa
    ...
    if countries_data:
        ...
        logger.info("=" * 80)
        logger.info("✅ COLETA COMPLETA DE COUNTRIES FINALIZADA!")
        logger.info(f"📊 Salvos: {saved}, Já existiam: {skipped}, Erros: {errors}")
        logger.info("=" * 80)
    else:
        logger.error("❌ Falha na coleta completa de countries")
