...
Objetivo: Enriquecer TODAS as fixtures restantes com múltiplos dados da API Sportmonks
Includes: referees,venue,league,season,state,round,stage,participants
Situação atual: 2.85% de cobertura de referees (1.915/67.085 fixtures)
Meta: Processar todas as 65.170 fixtures sem referee
"""
...
class FullScaleEnrichmentCollector:
    def __init__(self):
        Config.validate()
    ...
    def collect_full_scale_enrichment(self, progress_interval: int = 100):
        """Coletar múltiplos dados para TODAS as fixtures"""
...
def main():
    """Função principal"""
    collector = FullScaleEnrichmentCollector()
    ...
    success = collector.collect_full_scale_enrichment(progress_interval=100)
