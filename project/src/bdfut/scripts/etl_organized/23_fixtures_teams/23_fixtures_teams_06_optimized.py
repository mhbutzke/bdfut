#!/usr/bin/env python3
"""
Script OTIMIZADO para enriquecimento completo de TODAS as fixtures com home_team_id e away_team_id
Usa atualizações em lote (batch updates) para máxima performance
"""
...
class OptimizedFixturesTeamsEnrichment:
    """Classe OTIMIZADA para enriquecimento completo de fixtures com IDs dos times"""
    ...
    def get_all_fixtures(self, limit: int = 50, offset: int = 0) -> List[Dict]:
        """Buscar TODAS as fixtures (incluindo as que já têm team IDs)"""
        try:
    ...
    def batch_update_fixtures(self, updates: List[Dict]) -> int:
        ...
        try:
            ...
            if response.data:
                logger.info(f"✅ Lote de {len(response.data)} fixtures atualizado com sucesso")
    ...
    def process_fixtures_batch(self, fixtures: List[Dict]) -> int:
        """Processar um lote de fixtures com atualizações em lote"""
        if not fixtures:
            return 0
...
""")
    ...
    def run_optimized_enrichment(self):
        """Executar enriquecimento OTIMIZADO de TODAS as fixtures"""
        logger.info("🚀 Iniciando ENRIQUECIMENTO OTIMIZADO de TODAS as fixtures com team IDs...")
        logger.info("=" * 80)
        ...
        batch_size = 50  # Aumentado para 50 fixtures por lote
        report_interval = 500  # Relatório a cada 500 fixtures processadas
        offset = 0
        ...
        while offset < self.total_fixtures:
            ...
            # Rate limiting reduzido (0.5 segundo entre lotes)
            time.sleep(0.5)
...
def main():
    """Função principal"""
    try:
        enrichment = OptimizedFixturesTeamsEnrichment()
        enrichment.run_optimized_enrichment()
