#!/usr/bin/env python3
"""
Script otimizado para enriquecimento usando endpoint multi da API Sportmonks
Processa múltiplas fixtures por vez com includes corretos
"""
...
class MultiEnrichmentOptimized:
    def __init__(self):
        self.sportmonks = SportmonksClient()
        self.supabase = SupabaseClient()
        self.batch_size = 10  # Processar 10 fixtures por vez
        self.request_delay = 2  # 2 segundos entre requests para respeitar rate limit
    ...
    def get_fixtures_batch(self, limit: int = None):
        ...
        try:
            # Buscar fixtures finalizadas que podem precisar de enriquecimento
    ...
    def enrich_fixtures_batch(self, fixtures_batch):
        ...
        try:
            ...
            # Chamada multi com includes para eventos, lineups e estatísticas
            ...
            logger.info(f"   📊 Eventos: +{events_inserted}, Lineups: +{lineups_inserted}, Stats: +{stats_inserted}")
        ...
        try:
            ...
            for event in events_data:
                event_data = {
                    ...
                    'related_player_name': event.get('related_player', {}).get('name'),
    ...
    def run_enrichment(self, limit: int = None):
        ...
        logger.info("🚀 INICIANDO ENRIQUECIMENTO OTIMIZADO COM ENDPOINT MULTI")
        logger.info("=" * 60)
...
def main():
    """Função principal"""
    enrichment = MultiEnrichmentOptimized()
