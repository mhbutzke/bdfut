#!/usr/bin/env python3
"""
Task 2.5 - Enriquecimento de Lineups (IDs Simples)
==================================================

Versão que usa IDs sequenciais simples para evitar overflow
"""
...
def enrich_lineups_simple():
    """Enriquecer fixtures com lineups (IDs simples)"""
    print("🚀 Task 2.5 - ENRIQUECIMENTO DE LINEUPS (IDs SIMPLES)...")
    ...
    try:
        ...
        fixtures_result = supabase.client.table('fixtures').select('id,sportmonks_id').limit(5).execute()
        test_fixtures = fixtures_result.data
        
        print(f"📋 Testando com {len(test_fixtures)} fixtures")
        
        lineups_collected = 0
        local_id_counter = 1000000  # Começar com ID alto para evitar conflitos
            ...
            try:
                ...
                if lineups_response.status_code == 200:
                    ...
                    if lineups:
                        ...
                        # Processar lineups com IDs sequenciais simples
                        processed_lineups = []
