#!/usr/bin/env python3
"""
CLI principal do BDFut
=====================

Interface de linha de comando para o sistema ETL de dados de futebol.
"""
import click
import logging
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'bdfut/logs/etl_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
@click.group()
@click.version_option(version="2.0.0", prog_name="BDFut")
def main():
    """BDFut - Sistema ETL para dados de futebol da Sportmonks API"""
    try:
        Config.validate()
